  ...
  body: [
    ...
    DOCS.table(['Rule', 'Why it exists'], [
      ...
      ['<strong>Models carry dialect variants.</strong> <code>JSONType</code>, ' +
       '<code>BigIntType</code>, <code>TimestampTZ</code>.',
       'The test suite runs on SQLite. Break this and the suite stops being runnable without ' +
       'services &mdash; or, worse, keeps running while behaving differently from production. ' +
       '<code>TimestampTZ</code> is a <code>TypeDecorator</code> for that second reason: SQLite ' +
       'ignores <code>timezone=True</code> and returns naive datetimes, so code that subtracts ' +
       'two timestamps raised <code>TypeError</code> in tests while being correct against ' +
       'PostgreSQL. It now re-attaches UTC on load, on every dialect.'],
...
alembic upgrade head`, 'shell') +
      `<ol>
        ...
        (<code>0018</code> at the time of writing). The chain is linear and should stay that
        way.</li>
    ...
    DOCS.table(['Module', 'Covers'], [
      ...
      ['<code>test_changes.py</code> (46)',
       'The change state machine, pause attribution, the high-risk rollback rule and ' +
      ...
      ['<code>test_import_export.py</code> (37)',
       'Header aliasing, validation, duplicate detection, and that the CSV template’s example ' +
       'rows line up with its header'],
      ['<code>test_config.py</code> (9)',
       'Configuration guards, chiefly that production refuses to start without a supplied ' +
...
pytest -k "incident"`, 'shell'),
    ...
    <p><code>.github/workflows/ci.yml</code> runs on every push to <code>main</code>, on every pull
    request, and on demand. Three jobs, in parallel:</p>`,

    DOCS.table(['Job', 'What it runs'], [
      ['<strong>Backend tests</strong>',
       'Python 3.12, <code>pip install -r requirements.txt</code>, then ' +
       '<code>pytest -m "not network"</code>. No service containers: the suite provisions its own ' +
       'SQLite file. Chromium is not downloaded, because nothing in the suite launches a browser.'],
      ['<strong>Frontend lint and build</strong>',
       'Node 20, <code>npm ci</code> (falling back to <code>npm install</code> when no lock file is ' +
       'committed), then <code>npm run lint</code> and <code>npm run build</code>.'],
      ['<strong>Docker images build</strong>',
       'Builds both images with Buildx and a GitHub Actions layer cache, and pushes neither. It ' +
       'proves the Dockerfiles still assemble; publishing is a release concern. The frontend image ' +
       'builds from the repository root, because it bundles <code>docs/</code>.']
    ]),

    DOCS.callout('warn', 'Why this exists',
      '<p>It was added after two failures that had been sitting in the repository unnoticed, both ' +
      'caused by nothing running. <code>test_template_is_downloadable_and_reimportable</code> had ' +
      'been red since a migration added two columns to the CSV template’s header without ' +
      'updating its example rows. And <code>npm run lint</code> failed outright, because ESLint 9 ' +
      'reads <code>eslint.config.js</code> and the repository only had a legacy ' +
      '<code>.eslintrc.json</code>.</p>' +
      '<p>A test suite nobody runs is documentation with a worse failure mode: it reads as ' +
